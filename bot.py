...
def message_reply(message):
    ...
    if message.text == '<16':
        if int(time.time() // 1) - last_time_message[message.from_user.id] < 2:
            ...
            msg = bot.send_message(message.chat.id, 'У вас значительный дефицит массы тела😔. Вот ваш персональный рацион питания на неделю🥗🌮🌯!\n Выберите день недели:', reply_markup=markup_days_of_the_week)
    ...
    if message.text == '16-18,5':
        if int(time.time() // 1) - last_time_message[message.from_user.id] < 2:
            ...
            last_time_message[message.from_user.id] = int(time.time() // 1)
            markup_days_of_the_week.add(button_mn, button_ts, button_wd, button_th, button_fr, button_st, button_sn)
            msg = bot.send_message(message.chat.id, 'У вас дефицит массы тела😕. Вот ваш персональный рацион питания на неделю🥗🌮🌯!', reply_markup=markup_days_of_the_week)
            bot.register_next_step_handler(msg, show_deficit)
...
def show_deficit(message):
    if message.text == 'понедельник':
        ...
        bot.send_message(message.chat.id, 'Жизнь – это создание себя! Чтобы начать, нужно перестать говорить и начать делать! Желаем вам удачи!')
    ...
    if message.text == 'среда':
        ...
        bot.send_message(message.chat.id, 'У вас обязательно получится достичь желаемого результата! Также не забывайте про спорт — это жизнь! Если же все-таки возникнут проблемы, ваше моральное и физическое состояния ухудшатся, обратитесь обязательно к специалисту!')
...
def show_excess(message):
    if message.text == 'пн':
        bot.send_message(message.chat.id, racion('пн'), reply_markup=markup_imt_categories)
